"""Cross-subject stimulus decoding with RepTrace M-CCA alignment."""
...
from reptrace.metrics.classification import subject_level_signflip_summary
...
MCCA_TARGET_CENTERING_MODES = ("group_mean", "target_unsupervised")
DEFAULT_MCCA_COMPONENTS = 64
DEFAULT_MCCA_REGULARIZATION = 1e-6
DEFAULT_MCCA_SAMPLE_MODE = "class_repetition"
DEFAULT_MCCA_TARGET_CENTERING = "target_unsupervised"


@dataclass(frozen=True)
class CrossSubjectMCCAConfig:  # pylint: disable=too-many-instance-attributes
    """Parameters for LOSO cross-subject stimulus decoding with M-CCA."""
    ...
    mcca_components: int | float = DEFAULT_MCCA_COMPONENTS
    mcca_regularization: float = DEFAULT_MCCA_REGULARIZATION
    mcca_subject_pca_components: int | float | None = None
    mcca_sample_mode: str = DEFAULT_MCCA_SAMPLE_MODE
...
def evaluate_cross_subject_mcca(data_folder, participants, *, config=None, outer_participants=None, progress=None, label_shuffle_control=False, label_shuffle_seed=0):
    """Run fixed-pipeline LOSO stimulus decoding with M-CCA alignment.

    With the default ``target_calibration_trials_per_class=0``, the held-out
    participant is transformed by the average training-subject M-CCA projection.
    This is calibration-free with respect to target labels. Setting a positive
    target-calibration count estimates a target-specific M-CCA projection from
    those labeled target trials and excludes them from scoring.
    """

    config = _normalized_mcca_config(config or CrossSubjectMCCAConfig())
    ...
    if len(participants) < 3:
        raise ValueError("At least three participants are required for cross-subject M-CCA decoding.")
    ...
    for test_participant in outer_participants:
        ...
        outer_row, participant_predictions = _evaluate_mcca_outer_fold(
            train_sets,
            test_set,
            config,
            classifier_param,
    ...
    group_summary_rows = summarize_cross_subject_mcca(outer_rows, config=config)
...
def export_cross_subject_mcca(  # pylint: disable=too-many-arguments
    ...
    *,
    ...
    """Run cross-subject M-CCA decoding and write CSV artifacts."""
...
def summarize_cross_subject_mcca(outer_rows, *, config=None):
    """Summarize held-out participant scores for one M-CCA configuration."""
    ...
    selected_actual_components = Counter(int(row["mcca_actual_components"]) for row in outer_rows)
    return [
        {
            ...
            "mcca_actual_component_counts": _format_counter(selected_actual_components),
            "mcca_regularization": config.mcca_regularization,
            "mcca_subject_pca_components": config.mcca_subject_pca_components,
            "mcca_repetitions_per_class": config.mcca_repetitions_per_class,
            ...
            "one_sided_exact_sign_p_value": float(sign_summary["one_sided_exact_sign_p_value"]),
            "one_sided_signflip_p_value": float(sign_summary["one_sided_signflip_p_value"]),
        }
    ]


def _evaluate_mcca_outer_fold(train_sets, test_set, config, classifier_param, *, label_shuffle_seed=None):
    ...
    features_by_subject = dict(train_features_by_subject)
    labels_by_subject = dict(train_labels_by_subject)
    if config.target_calibration_trials_per_class > 0:
        ...
        features_by_subject[test_set.participant] = test_set.features[calibration_mask]
        labels_by_subject[test_set.participant] = np.asarray(test_set.labels, dtype=int)[calibration_mask]
    ...
    mcca_model, class_alignment = fit_class_mcca(
        features_by_subject,
        labels_by_subject,
    ...
    for feature_set in train_sets:
        transformed_train.append(mcca_model.transform(feature_set.participant, feature_set.features))
    ...
    if config.target_calibration_trials_per_class > 0:
        test_matrix = mcca_model.transform(test_set.participant, test_set.features[score_mask])
        ...
        test_matrix = mcca_model.transform_group(test_set.features[score_mask], feature_mean=target_mean)
    ...
    prediction_rows = _prediction_rows(
        ...
        target_transform=target_transform,
        mcca_actual_components=mcca_model.n_components,
...
def _outer_row(  # pylint: disable=too-many-arguments
    ...
    *,
    ...
    return {
        ...
        "mcca_sample_mode": config.mcca_sample_mode,
        "mcca_requested_components": config.mcca_components,
        "mcca_actual_components": int(mcca_model.n_components),
        "mcca_regularization": config.mcca_regularization,
        "mcca_subject_pca_components": config.mcca_subject_pca_components,
        "mcca_repetitions_per_class": class_alignment.n_repetitions_per_class,
        "mcca_alignment_classes": ",".join(str(int(value)) for value in class_alignment.classes),
        "target_transform": target_transform,
        "target_calibration_trials_per_class": config.target_calibration_trials_per_class,
        "target_centering": config.target_centering,
        "classifier": config.classifier,
    ...
    *,
    ...
    for output_index, (trial_index, true_label, predicted_label, confidence_score) in enumerate(zip(trial_indices, test_labels, predictions, confidence_scores)):
        row = {
            ...
            "alignment": _alignment_label(config),
            "mcca_sample_mode": config.mcca_sample_mode,
            "mcca_actual_components": int(mcca_actual_components),
            "target_transform": target_transform,
            "target_calibration_trials_per_class": config.target_calibration_trials_per_class,
            "target_centering": config.target_centering,
            "classifier": config.classifier,
...
def _topk_and_rank_metrics(true_labels, class_scores, score_classes):
    ...
    ranks = _true_label_ranks(true_labels, class_scores, score_classes)
    finite_ranks = np.asarray([rank for rank in ranks if np.isfinite(rank)], dtype=float)
...
def _normalized_mcca_config(config):
    ...
    if sample_mode not in CLASS_ALIGNMENT_SAMPLE_MODES:
        raise ValueError(f"Unsupported M-CCA sample mode: {config.mcca_sample_mode}. Supported modes: {', '.join(CLASS_ALIGNMENT_SAMPLE_MODES)}.")
    if target_centering not in MCCA_TARGET_CENTERING_MODES:
        raise ValueError(f"Unsupported target centering: {config.target_centering}. Supported modes: {', '.join(MCCA_TARGET_CENTERING_MODES)}.")
    if config.target_calibration_trials_per_class < 0:
    ...
    return CrossSubjectMCCAConfig(
        ...
        signflip_seed=config.signflip_seed,
        mcca_components=config.mcca_components,
        mcca_regularization=float(config.mcca_regularization),
        mcca_subject_pca_components=config.mcca_subject_pca_components,
        mcca_sample_mode=sample_mode,
        mcca_repetitions_per_class=config.mcca_repetitions_per_class,
...
def _alignment_label(config):
    if config.target_calibration_trials_per_class > 0:
        return "class_mcca_target_calibrated"
    return "class_mcca_group_average"
...
def _build_cross_subject_mcca_parser(prog: str | None = None) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog=prog, description="Run leave-one-subject-out stimulus decoding with RepTrace M-CCA alignment.")
    ...
    parser.add_argument("--components-pca", type=parse_int_or_inf, default=DEFAULT_CROSS_SUBJECT_COMPONENTS_PCA, help="Post-M-CCA PCA components for the classifier, or inf.")
    parser.add_argument("--mcca-components", type=parse_int_or_inf, default=DEFAULT_MCCA_COMPONENTS, help="Number of M-CCA components, or inf.")
    parser.add_argument("--mcca-regularization", type=float, default=DEFAULT_MCCA_REGULARIZATION, help="Ridge term for subject whitening.")
    parser.add_argument("--mcca-subject-pca-components", type=_parse_optional_int_or_inf, default=None, help="Optional within-subject whitening rank cap; use none or inf.")
    parser.add_argument("--mcca-sample-mode", choices=CLASS_ALIGNMENT_SAMPLE_MODES, default=DEFAULT_MCCA_SAMPLE_MODE, help="How to build aligned M-CCA rows from stimulus labels.")
    parser.add_argument("--mcca-repetitions-per-class", type=int, default=None, help="Optional cap for class_repetition alignment rows per class.")
    parser.add_argument(
        "--target-calibration-trials-per-class",
        type=int,
        default=0,
        help="Labeled held-out trials per class used only to fit the target M-CCA projection; excluded from scoring.",
    )
    parser.add_argument(
        "--target-centering",
        choices=MCCA_TARGET_CENTERING_MODES,
    ...
    parser.add_argument("--outer-output", default="outputs/stimulus_cross_subject_mcca_outer.csv", help="Held-out participant score CSV.")
    parser.add_argument("--summary-output", default="outputs/stimulus_cross_subject_mcca_group_summary.csv", help="Group summary CSV.")
    parser.add_argument("--predictions-output", default="outputs/stimulus_cross_subject_mcca_predictions.csv", help="Trial prediction CSV.")
    parser.add_argument("--confusion-output", default="outputs/stimulus_cross_subject_mcca_confusion.csv", help="Confusion-count CSV.")
    parser.add_argument("--per-stimulus-output", default="outputs/stimulus_cross_subject_mcca_per_stimulus.csv", help="Per-stimulus recall CSV.")
    parser.add_argument("--confusion-pairs-output", default="outputs/stimulus_cross_subject_mcca_confusion_pairs.csv", help="Bidirectional stimulus-pair confusion CSV.")
    return parser


def stimulus_cross_subject_mcca(argv: Sequence[str] | None = None, prog: str | None = None) -> int:
    """CLI entry point for fixed-pipeline cross-subject M-CCA decoding."""

    parser = _build_cross_subject_mcca_parser(prog=prog)
